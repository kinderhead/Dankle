...
namespace DankleTranslator
{
    public class IntelTokenizer(string input) : BaseTokenizer<Token, Token.Type>(input)
    {
        ...
        protected override void GenerateTokenMap()
        {
			...
			TokenMap[Token.Type.Register] = new(@"^(([a|b|c|d]x)|([d|e]s)|([s|b|i]p)|([s|d]i))\b");
			TokenMap[Token.Type.ByteRegister] = new(@"^([a|b|c|d]l)\b");
			TokenMap[Token.Type.Integer] = new(@"^(([0-9a-fA-F]+H)|\d+)");

			TokenMap[Token.Type.Ignore] = new(@"^((\.387)|EXTRN|ASSUME|DGROUP|GROUP|(CONST\d*)|ENDS|DB|END|SEGMENT|WORD|USE16|BYTE|_DATA|offset DGROUP:|far ptr|near ptr|word ptr)");
